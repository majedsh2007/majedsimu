...
// Migration: add column that tracks the single session currently
// allowed to use each account (single-device login).
try {
...
// Returns true only if req.sessionID is still the one and only session
// registered as "active" for this account. A newer login from another
// device overwrites active_session_id, which invalidates this one.
function isCurrentDeviceSession(req) {
  if (!req.session.user) return false;
  const row = db.prepare(
    "SELECT active_session_id FROM users WHERE id=?"
  ).get(req.session.user.id);
  return !!row && row.active_session_id === req.sessionID;
}

function invalidateSession(req,res,message) {
  req.session.destroy(() => {
    res.clearCookie("connect.sid");
    res.status(401).json({error: message, code:"SESSION_REPLACED"});
  });
}

function requireAuth(req,res,next) {
  if (!req.session.user) return res.redirect("/login.html");
  if (!isCurrentDeviceSession(req)) {
...
function requireAdmin(req,res,next) {
  ...
  if (!isCurrentDeviceSession(req))
    return invalidateSession(req,res,"تم تسجيل الدخول إلى هذا الحساب من جهاز آخر.");
  next();
}
...
app.post("/api/login", (req,res) => {
  ...
  req.session.regenerate(err => {
    ...
    // Registering this new session as the active one automatically
    // invalidates any session this account was using on another device.
    db.prepare("UPDATE users SET active_session_id=? WHERE id=?")
      .run(req.sessionID, user.id);
    req.session.save(err2 => {
      if (err2) return res.status(500).json({error:"تعذر إنشاء جلسة الدخول."});
...
app.post("/api/logout",(req,res)=>{
  if (req.session.user) {
    // Only clear it if this device is still the registered one, so a
    // stale/kicked-out session can't wipe out a newer login elsewhere.
    db.prepare(
      "UPDATE users SET active_session_id=NULL WHERE id=? AND active_session_id=?"
    ).run(req.session.user.id, req.sessionID);
  }
  req.session.destroy(()=>{ res.clearCookie("connect.sid"); res.json({ok:true}); });
});
app.get("/api/me",(req,res)=>{
  if (req.session.user && !isCurrentDeviceSession(req)) {
    return req.session.destroy(() => {
      res.clearCookie("connect.sid");
      res.json({authenticated:false,user:null,reason:"session_replaced"});
    });
  }
